...
//			저장후 이름 가져오기
		...
		ReViewDao.getInstance().ReViewUpdate(mapper,vo);		//정보 업데이트
		mapper.commit();
		
		vo = ReViewDao.getInstance().selectByIdx(mapper,idx);	// 고친 정보 다시 불러오고
//		System.out.println("바뀐 정보 vo : "+vo);
		//정보창에 전달할 현재페이지와 vo데이터
		request.setAttribute("currentPage", currentPage);
		request.setAttribute("vo", vo);
